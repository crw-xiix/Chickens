...
bool Gsender::Send(const String &to, const String &message)
{
...
#endif
  client.println(CREDS_EMAILBASE64_LOGIN);
  AwaitSMTPResponse(client);

#if defined(GS_SERIAL_LOG_2)
  Serial.println("EMAILBASE64_PASSWORD:");
#endif
  client.println(CREDS_EMAILBASE64_PASSWORD);
  if (!AwaitSMTPResponse(client, "235")) {
    _error = "SMTP AUTH error";
    return false;
  }
  
  String mailFrom = "MAIL FROM: <" + String(CREDS_FROM) + '>';
#if defined(GS_SERIAL_LOG_2)
  ...
  client.println("From: <" + String(CREDS_FROM) + '>');
